...
class PhysicsBody extends Node2d {
  ...
  getNearest(...layer) {
    ...
    layer.forEach((layer) => {
      for (let v of this.world._collisionLayer[layer]) {
        let target = v.getRect();
        let targetPos = new Vector(0, 0);
        targetPos.x = target.x + target.width / 2;
        targetPos.y = target.y + target.height / 2;
        let diffX = position.x - targetPos.x;
        let diffY = position.y - targetPos.y;
